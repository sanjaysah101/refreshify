...
const RotatingText = forwardRef<RotatingTextRef, RotatingTextProps>(
  (
    ...
    return (
      <motion.span
        className={cn("relative inline-flex flex-wrap pb-[10px] align-bottom whitespace-pre-wrap", mainClassName)}
        {...rest}
        layout
      >
        ...
        <AnimatePresence mode={animatePresenceMode} initial={animatePresenceInitial}>
          <motion.div
            ...
            className={cn(
              ...
              splitBy === "lines" ? "w-full flex-col items-start" : "flex-row items-baseline"
            )}
            layout
            aria-hidden="true"
            initial="initial"
  ...
  <AnimatePresence>
    {isOpen && (
      <motion.div
        ...
        className="absolute top-full left-1/2 z-40 mt-2 w-56 origin-top -translate-x-1/2 transform"
      >
        <div className="rounded-md border border-gray-700/50 bg-[#111111] p-2 shadow-xl">{children}</div>
      </motion.div>
    )}
  </AnimatePresence>
);
...
const DropdownItem: React.FC<DropdownItemProps> = ({ href = "#", children, icon }) => (
  <a
    href={href}
    className="group flex w-full items-center justify-between rounded-md px-3 py-2 text-sm text-gray-300 transition-colors duration-150 hover:bg-gray-700/30 hover:text-white"
  >
    <span>{children}</span>
    {icon &&
  ...
  return (
    <div className="relative flex min-h-screen flex-col overflow-x-hidden bg-[#111111] pt-[100px] text-gray-300">
      ...
      <main className="relative z-10 flex flex-grow flex-col items-center justify-center px-4 pt-8 pb-16 text-center">
        ...
        >
          <span className="text-xs font-medium tracking-wider text-gray-500 uppercase">Works with</span>
